...
作者：Claude（原始内容），Codex整理
日期：2026-04-24
## 数据结构复习:顺序表

本章介绍**顺序表**——线性表的顺序存储结构。顺序表既是一切动态数组的原型,也是线性表章节中的核心内容。

本章覆盖**静态顺序表**和**动态顺序表**两种形式,这也是考研教材(严蔚敏、王道)的标准分法。
...
在进入代码实现之前,先从整体上比较两者的差异。这部分也是常见的考研简答题考点：
...
静态和动态顺序表分别放在两个文件中,并使用 `SqList`(静态) 与 `DySqList`(动态) 进行命名区分,以避免两种实现相互冲突。
...
下面几个是**顺序表相关的经典考研题**,这里给出思路,可据此自行编写代码。每题都**配合顺序表的特性**来设计,这是这一节最值得训练的能力。
...
- 学**堆和堆排序**(优先队列的实现,基于完全二叉树数组存储,考研必考排序之一)。
- 学**图论**(邻接矩阵、邻接表、DFS、BFS、最小生成树 Prim/Kruskal、最短路径 Dijkstra/Floyd、拓扑排序)——**图是考研大题分值最高的章节之一**。
- 学**排序算法**(冒泡、选择、插入、希尔、快排、归并、堆排、基数、计数——考研必考,大题常出)。
- 学**查找算法**(线性查找、二分查找、分块查找、散列表)。
- 学 **B 树和 B+ 树**(磁盘索引核心)。
- 先答思考题再继续。
...
本章从链表开始,先给出项目组织框架,再依次说明链表的实现。

### 一、项目文件组织方案

建议创建如下目录结构,便于后续各类数据结构的学习：
...
下面给出可写入 `LinkedList/LinkedList.h` 的内容。
...
注意我在参数表里用了 `&L`(引用)。这一点十分关键：**当函数内部可能修改头指针本身(比如初始化、在第一个位置插入不带头链表)时,必须用引用传递**,否则修改只影响形参,调用者看不到变化。
...
本章系统介绍**栈和队列**的实现,分别给出**顺序存储**和**链式存储**两个版本。这部分内容是树的非递归遍历、图的 BFS/DFS 和表达式求值等后续主题的基础。
...
**链队列**用带头结点的单链表,设 `front` 指向头结点,`rear` 指向尾节点。加头结点是为了让空队和非空队的操作统一,这和链栈不同,需要特别注意。
...
**链队列最容易错的一个点**:在 `DeQueue_Lk` 中,当你删除的是最后一个数据节点时,`rear` 会变成悬空指针,必须让 `rear` 回到 `front`(头结点)。很多教材和博客的代码在这里有 bug,需要警惕。
...
- 基于栈实现应用题:括号匹配、表达式求值(中缀转后缀)、进制转换——这些是栈最经典的应用,考研大题常考。
- 基于队列实现应用题:用两个栈实现队列、用两个队列实现栈、队列解杨辉三角。
- 直接进入 **KMP 算法**(你最初提到要学的内容)。
- 回到二叉树,接着学线索二叉树或 BST。
...
本章介绍 KMP 算法。KMP 是串匹配中的经典算法,代码简洁,但理解重点在于原理推导,因此本章将重点说明其核心思想。
...
**这段代码本质是"模式串自己和自己做 KMP 匹配"**,用递推方式算出每个位置的 next 值。这里帮你模拟一遍 T = "ababaa"(长度 6):
...
- 继续回到**二叉树的线索二叉树**学习(这是下一次对话的自然延续)。
- 学**二叉搜索树 BST**(插入、查找、删除,尤其删除的三种情况最难)。
- 学**哈夫曼树和哈夫曼编码**(考研必考)。
- 学**堆**(用数组实现完全二叉树,堆排序的核心)。
- 先批改上面的思考题再继续。
...
本章介绍树这一重要的非线性结构。树是后续学习二叉树、图、平衡树和 B 树的重要基础,本文按照"概念 → 存储结构 → 代码实现 → 常用操作"的顺序展开。

在开始之前这里先说明：一般建议按照**先二叉树 → 再一般树和森林**的顺序学习,因为一般树最常用的存储方式之一是"孩子兄弟表示法",其本质就是把一般树转化为二叉树。

本章按照**一般树(普通树)**展开;二叉树以及树和森林的转换可在后续章节继续学习。
...
一般树的存储比二叉树复杂,因为每个节点的孩子数不固定。教材上有三种经典方案,都需要掌握:
...
可进一步学习:(A) 先让我解答上面三个思考题再继续;(B) 直接开始"二叉树"专题,包括二叉树的定义、四种遍历(先中后层)、线索二叉树、二叉搜索树等;(C) 学习"树、森林、二叉树的相互转换"算法;(D) 补充一般树的双亲表示法和孩子表示法的完整代码实现。
...
本章介绍**二叉树**。二叉树是数据结构中的核心章节,后续的 BST、AVL、红黑树、堆、哈夫曼树和 B 树等内容都与其密切相关。本文按"概念 → 存储 → 遍历 → 线索化 → BST"的顺序展开。

本章内容较多,这里先介绍**二叉树的概念、两种存储结构、四种遍历(递归+非递归)以及常用操作**;线索二叉树和 BST 可放在后续章节继续学习。
...
- 线索二叉树(中序线索化及其遍历,考研高频);
- 二叉搜索树 BST(插入、查找、删除,尤其删除的三种情况);
- 二叉树进阶题(构造二叉树 from 前序+中序、判断对称、最近公共祖先等)。

---

## 用自己写的栈和队列重做二叉树的非递归遍历 & 层序遍历

本章将使用自定义实现的 `SqStack` 和 `LkQueue` 重做二叉树的非递归遍历与层序遍历。这一练习有助于把栈、队列的底层实现与上层算法真正关联起来。
...
这里额外加入了 `DestroyQueue_Bi(Q)`。由于链队列的头结点通过 `new` 动态申请,若不销毁会造成内存泄漏;顺序栈则是局部变量,函数结束时可自动释放。二者在内存管理上的区别需要注意。
...
如果使用 IDE(VS / CLion / VS Code),将这四个 .cpp 文件加入项目即可。
...
- 学**线索二叉树**(考研高频,要用到中序遍历思想)。
- 学**二叉搜索树 BST**(插入、查找、删除)。
- 学**哈夫曼树**(优先队列/最小堆应用)。
- 学**树、森林与二叉树的相互转换**(你最初列表中的内容)。
- 先回答思考题,我批改。
...
本章介绍**线索二叉树**。线索二叉树是二叉树章节中较为抽象的一部分,本章将重点说明其设计动机、基本规则与代码实现。

需要注意的是,线索二叉树的代码本身不复杂,但难点在于理解线索化过程的实际含义。建议先跟随推导理解结构变化,再回看代码实现。
...
- 学**二叉搜索树 BST**(插入、查找、删除的三种情况,考研大题高频)。
- 学**哈夫曼树和哈夫曼编码**(考研必考,应用题能出选择也能出大题)。
- 学**树、森林与二叉树的相互转换**(你最初学习清单里的内容,很适合在这里串起来)。
- 用你自己的栈重写先序/后序线索化的遍历代码作为练习。
- 先答思考题再继续。
...
本章介绍树、森林与二叉树的相互转换。这一内容虽然常被忽视,但在理论上十分重要,因为它揭示了普通树、森林与二叉树之间的内在联系。

本章将先详细说明转换规则,再给出代码实现。树、森林与二叉树相互转换的真正重点在于规则理解,代码实现反而相对直接。

---

### 一、核心洞察(先建立直觉)

回忆一下若已学习过的"孩子兄弟表示法":每个节点只有两个指针,一个指向**第一个孩子**,一个指向**下一个兄弟**。
...
- 学**二叉搜索树 BST**(插入、查找、重点是删除的三种情况)。
- 学**哈夫曼树和哈夫曼编码**(考研大题高频,构造过程是必会的)。
- 学**堆和堆排序**(完全二叉树的数组存储应用)。
- 回到前面还没做完的**顺序表**章节(你最初列的清单里还有这一项)。
- 先答思考题再继续。
...
本章介绍**哈夫曼树**(Huffman Tree)。哈夫曼树是经典的应用型数据结构,核心问题是如何用尽量少的比特数对文本进行编码。

本章的特点是概念本身不复杂,但手工构造过程容易出错,代码实现也有较强的套路性。本文按"问题引入 → 手工构造 → 代码实现 → 编码译码"的顺序展开。
...
- 学**二叉搜索树 BST**(插入、查找、删除的三种情况,考研大题高频)。
- 回到你最初列表里的 **顺序表**(线性表的顺序存储结构,相对基础但必须会)。
- 学**堆和堆排序**(堆其实是一种特殊的完全二叉树,也可以优化哈夫曼树的合并过程)。
- 学**平衡二叉树 AVL**(BST 进阶,插入时的四种旋转)。
- 先答思考题再继续。
...
本章介绍**二叉搜索树**(Binary Search Tree,简称 BST),也称"二叉排序树"。BST 是重要的动态查找结构,常见考点包括插入、删除、查找及效率分析。

这一章最大的难点是**删除操作的三种情况**,特别是"删除度为 2 的节点"时的处理。本文将把每种情况都讲透。
...
- 学**平衡二叉树 AVL**(BST 的升级版,通过旋转保证 O(log n) 的高度,考研大题超高频,特别是四种旋转:LL、RR、LR、RL)。
- 学**堆和堆排序**(优先队列的核心,基于完全二叉树数组存储)。
- 回到你最初列表里还没做的**顺序表**(线性表的顺序存储)。
- 学**图论基础**(邻接矩阵、邻接表、DFS、BFS)。
- 先答思考题再继续。
...
本章介绍**平衡二叉树**(AVL Tree)。AVL 树是 BST 的重要改进结构,其核心在于通过旋转保持树高平衡。

**事先提醒**:AVL 的**代码不算太难,但四种旋转的判断条件和旋转后的形态极其容易混淆**。本文将把每种情况都配图讲清,建议配合纸笔画图理解,仅靠阅读往往难以准确掌握其变化过程。
...
- 学**堆和堆排序**(基于完全二叉树数组存储,堆排是考研必考排序之一)。
- 学**B 树和 B+ 树**(多路平衡树,数据库索引的核心)。
- 学**红黑树**(C++ `std::map` 底层,比 AVL 更实用但旋转更复杂)。
- 回到你最初列表里还没做的**顺序表**。
- 进入**图论**(邻接矩阵、邻接表、DFS、BFS、最小生成树、最短路径)。
- 先答思考题再继续。
...
本章介绍 **B 树和 B+ 树**。这两种结构是数据库索引中的核心数据结构,也是 BST/AVL 在磁盘存储场景下的自然延伸。
...
B+ 树的完整代码实现在考研范围内不作要求,**重点是理解其结构和与 B 树的差异**。如需完整实现,可在此基础上继续展开。
...
- 学**图论**(DFS、BFS、最小生成树、最短路径、拓扑排序——考研大题分值最高的章节,也是面试最常考的)。
- 学**排序算法**(快排、归并、堆排序、希尔、基数——考研必考,每年必有一道大题)。
- 学**查找算法**(散列表/哈希表——和 B+ 树是数据库两大索引结构,面试高频)。
- 学**堆和堆排序**(既是排序算法,又是优先队列的基础)。
- 先答思考题再继续。
...
堆建立在完全二叉树的数组存储之上,因此在学习堆之前,需要先掌握以下基础内容:

**第一,完全二叉树与满二叉树的区别**。完全二叉树要求除最后一层外其余各层都满,且最后一层节点从左到右连续排列;满二叉树则要求每一层都达到最大节点数。

**第二,完全二叉树适合用数组存储**。由于节点在层序下排列紧凑,可以直接通过下标推导父子关系,无需额外指针。

**第三,数组表示中的父子下标关系**。若采用 `a[1..n]` 存储:

- 父节点下标为 `i / 2`
- 左孩子下标为 `2 * i`
- 右孩子下标为 `2 * i + 1`

**第四,最后一个非叶子节点的位置**。对于含 `n` 个节点的完全二叉树,最后一个非叶子节点的下标为 `n / 2`。这一结论直接决定了建堆时的起始位置。

---

### 二、这些前置知识的重要性

堆排序的代码本身并不长,核心通常只有建堆和调整两个关键过程,但它的每一步都依赖前面的基础知识:
...
因此,在正式学习堆之前,先梳理这些基础概念,可以显著降低后续理解代码和推导过程的难度。
...
按照这一顺序学习,可以将堆的概念、结构、操作与应用自然串联起来,形成完整的知识框架。

---

## 数据结构复习:堆和堆排序

本章按照**概念铺垫 → 存储结构 → 核心操作 → 应用 → 思考题**的顺序介绍堆和堆排序。虽然代码量不大,但各步骤背后的原理必须理解透彻。
...
- **排序算法全家桶**(冒泡、选择、插入、希尔、快排、归并、基数、计数——考研必考,内部排序是整章大题)。堆排在此作为一员已经学完。
- **图论**(邻接矩阵、邻接表、DFS、BFS、Prim/Kruskal、Dijkstra、Floyd、拓扑排序、关键路径)——考研分值最高的章节之一,且 Dijkstra 会用到优先队列(堆的应用)。
- **散列表 / 哈希表**(开放定址法、链地址法、冲突处理、装填因子、性能分析)——和 B+ 树并列数据库索引两大支柱。
- 先答思考题再继续。
...
本章介绍**红黑树**(Red-Black Tree)。红黑树是工业界极为重要的平衡搜索树,广泛用于 `std::map`、`std::set` 等有序关联结构的实现。
...
- **散列表 / 哈希表**(O(1) 查找的另一条路,与红黑树/B+树并列的重要索引结构,考研必考,代码易写概念易混)。
- **图论入门**(DFS、BFS、邻接矩阵、邻接表——考研分值最高章节的开端)。
- **排序算法全家桶**(堆排如果已经学习过了,还差快排、归并、希尔、基数等——考研必有一道综合大题)。
- **跳表**(概率性的有序结构,Redis 在用,面试爱考)。
- 先答思考题再继续。
...
本章介绍**图论**。图是数据结构中分值高、代码量大且概念密集的重要章节,大量实际问题都可以抽象为图模型。
...
- **图的遍历**——DFS(深度优先)和 BFS(广度优先)。**这是所有图算法的起点**,也是考研大题最常考的基础。本文将分别给出**邻接矩阵版**和**邻接表版**的代码,还会引入**visited 数组**这个图遍历的灵魂。

- 先做思考题再推进。

- 如果你想快进,也可以先看**最小生成树**(Prim 和 Kruskal)或**最短路径**(Dijkstra 和 Floyd),但我建议按顺序——DFS/BFS 是后面所有算法的前置。

---

## 数据结构复习:图的遍历(DFS 和 BFS)

本章介绍**图的遍历**。DFS 和 BFS 是理解后续最小生成树、最短路径、拓扑排序和关键路径等算法的重要基础。

本章按**为什么图遍历需要 visited 数组 → DFS 的递归/非递归实现 → BFS 实现 → 连通分量应用**的顺序展开,并同时给出邻接矩阵和邻接表两种存储结构下的代码实现。
...
**图不一样。图有环,有回路**。如果从顶点 A 出发,走到 B,再走到 C,C 可能有一条边指回 A。若不记录"A 已访问",遍历过程就会在 A-B-C-A-B-C 中**无限循环**。
...
这两个算法是图论的基石,**后续几乎所有算法都是它们的变种**。先给出一个"路线图",接下来的章节你就知道每个算法为什么存在:
...
- **最小生成树**(Prim 和 Kruskal)——实际意义非常强(网络布线成本最小化),代码不长但概念经典,**考研大题必考点之一**。

- **最短路径**(Dijkstra 和 Floyd)——GPS 导航的核心算法,考研大题高频。Dijkstra 可以看作"带权 BFS"。

- **拓扑排序 + 关键路径**(AOV 和 AOE 网)——项目管理、任务调度的核心。基于 DFS 或 BFS 入度法。

- 先答思考题再继续。

**建议的学习顺序是 (C) → (A) → (B)**,因为拓扑排序最贴近 DFS/BFS(几乎就是 DFS/BFS 的直接应用),Prim/Dijkstra 则需要引入"带权"和"贪心"的新概念,循序渐进更容易。但你可以按自己的节奏选。

---

## 数据结构复习:拓扑排序与关键路径

本章介绍**拓扑排序**和**关键路径**。这两个主题与 DFS/BFS 紧密相关,并在编译系统、构建系统和项目管理中有直接应用。
...
- **最小生成树**(Prim 和 Kruskal)——带权无向图的经典问题,应用于网络布线、电路设计,**考研大题高频**。Prim 基于贪心 + BFS,Kruskal 基于边排序 + 并查集(顺便学并查集)。

- **最短路径**(Dijkstra 和 Floyd)——GPS 导航、网络路由的核心。Dijkstra 用"优先队列(堆)"加速——**正好把前面堆的内容应用起来**!Floyd 用三重循环的动态规划,代码极短但思想深刻。
...
本章介绍**最小生成树**(Minimum Spanning Tree, MST)。最小生成树问题概念直观、算法经典,同时也会涉及 Kruskal 算法常用的并查集结构。
...
**第一题**,下面这张带权无向图,分别用 Prim(从 A 出发)和 Kruskal 构造 MST,**列出每一步的边和最终总权**。可以发现两种算法选的边可能不完全一样,但总权一定相等:
...
- **最短路径**(Dijkstra 和 Floyd)——压轴章节,和 MST 并列为图论的"双子星"。Dijkstra 和 Prim 代码几乎一模一样,只差一个细节;Floyd 是三重循环的动态规划,代码极短却思想深刻。**考研图论大题一半以上都是最短路径**。

- **强连通分量**(Tarjan / Kosaraju)——比较进阶,考研较少考,但面试爱问。
...
本章介绍图论中的**最短路径**问题。最短路径是图论中的核心内容,也是导航、网络路由等应用场景中的基础算法。

本节介绍两种经典算法:**Dijkstra**(单源最短路径)和 **Floyd**(多源最短路径)。二者思路和代码风格差异很大,但都体现了最短路径问题中的核心思想;其中 Dijkstra 与 Prim 在写法上也有明显的相似性。
...
**到这里,已经掌握了考研 408 数据结构 80% 以上的内容**——只剩**查找**(散列表)和**排序**(快排、归并等)这两个章节。

---

### 十一、下一步方向

- **排序算法全家桶**——冒泡、选择、插入、希尔、快排、归并、堆排(你已会)、基数、计数。**考研必有大题**,且每种排序都是独立小世界,代码精巧。

- **散列表(哈希表)**——与 B+ 树并列的"数据库索引两大支柱",平均 O(1) 查找,面试爱考。涉及开放定址法、链地址法、装填因子、冲突处理。

- **跳表**(概率性有序结构,Redis 在用)或**字典树 Trie**(字符串处理利器)——拓展内容,考研不要求,面试偶尔考。
...
本章介绍**排序算法**。排序是数据结构课程中的核心大章节,也是考研和面试中的高频考点。
...
**每种排序我都按这四点讲**,应形成"每种排序 = 一张小卡片"的记忆。
...
这是**每年必考**的选择题类型。需要熟练手工模拟每种排序的一趟过程。
...
给你原始数组和几个候选,问哪个是某种排序过程中的中间状态。需要根据"每轮结束后数组形态的特征"判断:
...
- 对本次四种排序还有疑问,想再展开讲某一种。

---

## 数据结构复习:排序算法(下)

本章继续介绍排序章节的后半部分,包括**选择类排序**(简单选择与堆排序回顾)、**归并排序**、**基数 / 桶 / 计数排序**,并在结尾给出综合比较与选择策略。

在前文已经介绍插入类、冒泡和快速排序等**基于比较**的排序方法。本章继续介绍**归并排序**(经典稳定的 O(n log n) 排序)以及**基数类排序**(突破比较模型限制的线性时间排序)。
...
**对稳定性、复杂度、适用场景都有清晰认识**。**排序这一章,考研 100% 命中率**——每年必考,已经武装到位。
...
**这几乎覆盖了考研 408 数据结构的全部内容**——**已经具备数据结构课程的完整知识图景**!剩下的只有"**查找**"章节中的**散列表**部分(BST、B+ 树也属于查找,已学过)。

---

### 十二、下一步

- **散列表(哈希表)**——查找部分的收官。开放定址法、链地址法、冲突处理、装填因子、性能分析。学完后你的**整本数据结构**就完整了。

- **更高阶的专题**——比如跳表、Trie 树、LRU 缓存、并查集的更多应用,面试方向。

- **综合实战**——给一个真实问题(比如"实现一个简单数据库"、"实现 LeetCode 风格的算法题"),用如果已经学习过的数据结构综合解决。
...
本章介绍**散列表**,这是查找部分的重要内容,也是数据结构学习中的收束章节之一。
...
假设若要存 100 个员工信息,员工编号 1~100。最简单的做法:**开一个 101 大小的数组,编号 i 的员工放在 a[i]**。查找 O(1),插入 O(1)——完美。
...
**第十题**,**散列表不支持有序遍历**。如果如果需要一个"支持快速查找 + 支持按 key 顺序遍历"的结构,你会选什么?(提示:这就是 C++ `map` 和 `unordered_map` 的区别)
...
**(D) 综合项目**:用已学的数据结构实现一个**小数据库**、**小编译器**、**LRU 缓存**——把知识串起来。

**(E) 先答思考题**:把散列表这章的思考题做完,我批改。

---

至此,本轮数据结构整理工作已形成较完整的知识框架。

---

## 数据结构专题深入:三选一路线图

本章给出数据结构**专题深入**的三条学习路线。这些专题虽然不一定属于基础考试大纲的核心范围,但在面试、竞赛和工业开发中都很重要。

---

### 一、三个专题的定位

以下给出三个专题的简要画像,便于进行理性选择：
...
**学习收获**:可以发现"很多看似不是图的问题,其实都能建模成图"——这是算法的最高境界。
...
- **B 树深化**:已学过 B 树基础,这里是看它如何真正地在**磁盘**上工作。
...
若对三条路线进行排序,推荐优先学习字符串专题,理由如下：

**理由一**:**性价比最高**。Trie 树本身就是面试高频,学完立刻能用。AC 自动机作为拓展,是"Trie + KMP"的美妙结合——把已学的 KMP 与新学的 Trie 串联起来,形成**知识的联动**。
...
第一,**不要像前面那样期待"每个细节都讲到"**。专题内容更深,本文将说明核心思想和代码框架,**具体细节和优化变体需要你动手写、动手试**。
...
后续可根据兴趣、时间与目标从以下方向展开：

**(A) 字符串专题**:Trie → AC 自动机 → 后缀数组(可选)。**推荐首选**,适合大多数人。

**(B) 高级图论专题**:Tarjan 强连通分量 → 二分图匹配 → 网络流。适合对图论特别感兴趣或计划走算法竞赛方向的读者。

**(C) 外存专题**:外部排序 → B 树实战 → LSM 树。适合计划走后端/数据库方向且希望理解工业系统的读者。
...
在这一阶段已经**覆盖了完整的数据结构课程大纲**——链表、顺序表、栈、队列、KMP、各类树、图、排序、散列表——这是一个**非常扎实的底子**。当前所处的学习位置,比很多"学过数据结构但没真正掌握"的同学要高出一大截。

专题深入**不是必须的**。如果考研在即、时间紧张,或者只是希望巩固已学内容,**先暂停专题、去刷题**也是非常好的选择——**知识要"消化"**,光输入不输出就会忘。

---

## 刷题计划：算法竞赛方向（从零开始）

本文给出一条面向**算法竞赛 + 几乎零基础 + 每天 1 小时**学习条件的入门路线。

需要先明确一个关键认识：**算法竞赛的核心不是"刷过多少题"，而是"每道题真正懂了没有"**。每天 1 小时，坚持 6 个月，认真做 150 道题，远好于走马观花做 500 道。
...
但好消息是：**已经有了别人没有的底子**——完整的数据结构知识图景，让你在竞赛里编写链表、树、图的代码时比别人快很多。
...
**目标**：熟悉竞赛 C++ 的写法，把如果已经学习过的数据结构和 LeetCode / 洛谷的题连起来。
...
这些写法并非坏习惯,而是竞赛环境下必要且标准的写法,需要熟练掌握。

**第二件事：熟悉 STL**。竞赛中频繁使用：

`vector`、`stack`、`queue`、`priority_queue`、`map`、`set`、`unordered_map`——**这些直接替代你手写的数据结构**，竞赛中不需要手写链表、不需要手写栈。学习手写实现的价值在于"理解底层"，实战中直接用 STL。
...
206 反转链表（最经典）、21 合并两个有序链表、141 环形链表（判环，如果已经学习过 Floyd 判环）、83 删除排序链表中的重复元素、2 两数相加。

**栈类（5 道）**：

20 有效的括号（括号匹配，就是已学栈时讲的应用）、155 最小栈、739 每日温度、84 柱状图中最大的矩形、394 字符串解码。
...
226 翻转二叉树、101 对称二叉树、112 路径总和、105 从前序与中序遍历序列构造二叉树（如果已经学习过，直接上手）、236 二叉树的最近公共祖先。
...
98 验证二叉搜索树、700 二叉搜索树中的搜索、701 二叉搜索树中的插入操作、450 删除二叉搜索树中的节点（就是已学的三种删除情况！直接上手）、230 二叉搜索树中第 K 小的元素。

**图类（8 道）**：

797 所有可能的路径（DFS 基础）、200 岛屿数量（经典 BFS/DFS）、695 岛屿的最大面积、207 课程表（**拓扑排序**，就是已学的 Kahn 算法！）、210 课程表 II（拓扑排序 + 路径）、743 网络延迟时间（**Dijkstra**，就是已学的！）、1584 连接所有点的最小费用（**Prim 或 Kruskal**，就是已学的！）、547 省份数量（并查集）。

**预计时间**：五周，每天 1 小时，约 23 道题。这个阶段可以发现：**原来学过的算法直接就能用上**，成就感很强。
...
这道题已学链表时我讲过原理，现在用竞赛 C++ 写出来。**要求**：写两个版本，递归版和迭代版各一个，然后提交通过。

第三，完成后应自行复查代码，重点检查写法是否规范以及边界情况是否遗漏。
...
本文件用于记录各专题的学习与整理进度。每完成一个专题,即可将对应条目标记为已完成,并按需要继续补充新的专题名称。
